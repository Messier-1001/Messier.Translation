...
class ArraySource extends AbstractSource
{
    ...
    *
    *
    * @param \Messier\Translation\Locale $locale The source Locale
   ...
   public function getTranslation( $identifier, ?string $defaultTranslation = null ) : ?string
   {
      ...
      if ( $this->_useNumericId )
      {
         ...
         {
            // Not integer identifier, return the default translation…
   ...
   public function getTranslations( ?string $category = null ) : array
   {
      ...
      if ( '' !== $category && ! \in_array( $category, $this->_categories, true ) )
      {
         return $translations;
      }
      ...
      foreach ( $this->_translations as $identifier => $transData )
      {
         ...
         if ( $withCategory )
         {
            ...
            if ( ! \is_array( $transData ) || ! isset( $transData[ 'category' ] ) ||
                 ! isset( $transData[ 'text' ] ) || $transData[ 'category' ] !== $category )
            {
               continue;
            }
            $translations[ $identifier ] = $transData[ 'text' ];
         ...
         else
         {
            if ( \is_array( $transData ) && isset( $transData[ 'text' ] ) )
            {
               if ( ! isset( $transData[ 'category' ] ) ||
                  ( isset( $transData[ 'category' ] ) && $transData[ 'category' ] === '' ) )
               {
                  $translations[ $identifier ] = $transData[ 'text' ];
               }
            }
   ...
   public function reload() : ISource
   {
      ...
      {
         $this->_translations = $this->options[ 'data' ];
      }

      if ( isset( $this->options[ 'file' ] ) && \file_exists( $this->options[ 'file' ] ) )
      {

         try
         {
            /** @noinspection PhpIncludeInspection */
            $translations = include $this->options[ 'file' ];
         }
    ...
    *
    * @param  string                      $folder
    * @param  \Messier\Translation\Locale $locale
    * @param  bool                        $useNumericId
    * @return \Messier\Translation\Source\ArraySource
    */
